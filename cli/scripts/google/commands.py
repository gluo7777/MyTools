...
REQUIRED_KEYS = [
    ('Google API Client Id', props.CLIENT_ID, True,
     False), ('Google API Client Secret', props.CLIENT_SECRET, True, False)
]
...
def oauth2(scopes):
    ...
    click.echo(
        f'Paste this into your browser to authorize this tool\n\n{consent_url}\n')
    redirect_url = click.prompt(
        'Now paste the url that appears after redirecting', type=str)
...
def revoke():
    ...
    choice = click.prompt('Are you sure you want to remove your credentials?', type=click.Choice(
        ['yes', 'no']), default='no', show_choices=True, show_default=False)
    if choice == 'yes':
