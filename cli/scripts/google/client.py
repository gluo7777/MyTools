...
class Client(client.Client):
    ...
    def access_token(self, refresh: bool = False) -> str:
        payload = {
            'client_id': self.props.get(self.props.CLIENT_ID), 'client_secret': self.props.get(self.props.CLIENT_SECRET), 'redirect_uri': self.REDIRECT
        }
        if refresh:
        ...
        response = requests.post(
            url=self.TOKEN_URL, headers=self._application_x_www_form_urlencoded(), data=payload, timeout=self._timeout
        )
        if response.ok:
            with client.Json(response) as body:
                return {
                    'access_token': body.get('access_token'), 'refresh_token': body.get('refresh_token'), 'expiration': body.get('expires_in')
                }
        else:
    ...
    def revoke_access(self):
        response = requests.post(
            url=self.REVOKE_URL, headers=self._application_x_www_form_urlencoded(), query={'token': self.props.get(self.props.ACCESS_TOKEN)}, timeout=self._timeout
        )
        with client.Json(response) as body:
            if not response.ok:
                raise GoogleException(
                    f"Failed to request access token:\n{body}")
...
def refresh_token(minutes: int = 15):
    ...
    def decorator(func):
        ...
        def wrapped_func(*args, **kwargs):
            self = cast(args[0])
            remaining_seconds = int(self.props.get(
                self.props.EXPIRATION, '0')) - time.time()
            if remaining_seconds < seconds:
                ...
                if not access_token:
                    raise GoogleException(
                        'Error in attempt to refresh access token')
