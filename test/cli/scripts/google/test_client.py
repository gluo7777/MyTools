...
class ClientTest(test.DeleteResourceTest):
    ...
    def test__authorization_header(self):
        ...
        self.assertEqual(self.client._authorization_header()['Authorization'],'Bearer abcd')
        self.props.get.assert_called_once_with(GoogleProperties.ACCESS_TOKEN)

    def test__application_x_www_form_urlencoded(self):
        self.assertEqual(self.client._application_x_www_form_urlencoded()['Content-Type'],'application/x-www-form-urlencoded')

    def test__transform_scopes(self):
        self.client.API = 'www.google.com'
        full_scopes = self.client._transform_scopes(['test','tasks','email'])
        self.assertEqual(
            "www.google.com/auth/test,www.google.com/auth/tasks,www.google.com/auth/email"
            ,full_scopes
        )

    def test_consent_url(self):
        self.client._transform_scopes = m.Mock(return_value='www.google.com/auth/test')
        ...
        self.client.props.get.assert_called_once_with(GoogleProperties.CLIENT_ID)

    def test_extract_code_from_url(self):
        code = self.client.extract_code_from_url(r'https://www.google.com/?state=lefoiiforji43joi3joi43jfoi3&code=4/xQHwVU_ANFz94g-SOSmU-JqVdYEc2L1uUp5fF0BUMn1YFXxyTLz3-nTxoEe30dm0bulZO6bMO4c-C7GfeZ5WGXk&scope=https://www.googleapis.com/auth/tasks')
        self.assertEqual(code, r'4/xQHwVU_ANFz94g-SOSmU-JqVdYEc2L1uUp5fF0BUMn1YFXxyTLz3-nTxoEe30dm0bulZO6bMO4c-C7GfeZ5WGXk')
